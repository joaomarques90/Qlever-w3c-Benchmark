...
def check_hierarchy(in_type: str, expected_type: str) -> (bool, str):

    type_list = ['iri', 'uri', 'url', 'urn']
    '''
    ...
    if expected_type not in type_list:
        return False, "Unknown expected type: \'" + expected_type + "\'"
    if in_type not in type_list:
        return False, "Unknown input type: \'" + in_type + "\'"

    if in_type == expected_type:
        return True, None
    elif in_type == 'iri':
        return True, "expected subset-type: \'" + expected_type + "\' but received superset-type: \'iri\'"
    elif in_type == 'uri' and expected_type == 'url':
        return True, "expected subset-type: \'url\' but received superset-type: \'uri\'"
    elif in_type == 'uri' and expected_type == 'urn':
        return True, "expected subset-type: \'urn\' but received superset-type: \'uri\'"
    else:
        return False, "expected type: \'" + expected_type + "\' but received type: \'" + in_type + "\'"
...
def check_row_sparql_json(variables: List[str], gold_row: List[Any], actual_row: Dict[str, any],
    ...
    for i, gold in enumerate(gold_row):
        ...
        original_var = var
        if var not in gold:
            eprint("\tColumn variable (" + var + ") not present.\n")
            return False
        if var not in actual_row and var.startswith("TEXT("):
            var = var[5:-1]
        # retrieves the value from the result-variable
        if gold[original_var] is None:
            # better logic for not skipping false-matches when a given-row-column is not None
            # fixed partial-match (no false-positive)
            if var in actual_row:
                actual = actual_row[var]
                if "value" in actual and actual["value"] is not None:
                    return False
            continue
        actual = actual_row[var]
        gold = gold[original_var]
        ...
        if 'type' not in gold:
            ...
            return False
        elif gold["type"] not in type_list:
            if not gold["type"] == "literal":
                eprint("\tUnknown Column type: " + gold["type"] + "\n")
        gold_type = gold["type"]
        # checks if variable type is a correct 'iri'
        if gold_type in type_list and not (gold_value[0] == '<' and gold_value[-1] == '>'):
        ...
        # check if type is a superset of the actual row
        if gold_type in type_list:
            (booleano, msg) = check_hierarchy(gold_type, actual["type"])
            if booleano and msg:
                wprint(msg)
            elif msg:
                eprint(msg)
                return False
        elif not gold_type == actual["type"]:
            return False
